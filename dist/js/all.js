...
$(document).ready(function () {
    ...
    function splitWeeks(start, end) {
        ...
        var Mstart = moment(new Date(start));
    ...
    var month = '03/2018';
    var currentMonth = moment().set({ year: month.split('/').pop(), month: month.split('/').shift() - 1 }).utc();
    var Dm = currentMonth.endOf('month').get('date');
    var count = 0;
    for (var i = currentMonth.endOf('month').get('date'); i > 0; i--) {
        var day = console.log(currentMonth.add(-1, 'days'));
        console.log('day', day);
        if (day > 0 && day < 6) {
            count += 1;
        }
